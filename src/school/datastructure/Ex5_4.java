...
class OperatePoly {
    ...
    public Polynomial addPoly(Polynomial A, Polynomial B) {
        degree_A = A.getDegree();       //3
        degree_B = B.getDegree();       //4
        expo_A = degree_A;              //expoA=3
        expo_B = degree_B;              //expoB=4
        if(degree_A >= degree_B) degree_C=degree_A;     //if(3>=4) => C=4
        else degree_C=degree_B;
        Polynomial C = new Polynomial(degree_C);
        while(index_A <= degree_A && index_B <= degree_B){          //0<=3 && 0<=4
            if(expo_A > expo_B) {
